...
namespace StressdetectionViaFace.LBPvariants
{
    class NI_LBP
    {
        ...
        // get tc 
        private double Gettc (int X,int Y)
        {
            // ave =get the sum of  elements / 8
   
            // get the  adjescent values 
            double[] values = new double[9];
            values[0] = GetPixelValue(X - 1, Y - 1);
            values[1] = GetPixelValue(X - 1, Y );
            values[2] = GetPixelValue(X - 1, Y + 1);
            values[3] = GetPixelValue(X , Y + 1);
            values[4] = GetPixelValue(X + 1, Y + 1);
            values[5] = GetPixelValue(X + 1, Y  );
            values[6] = GetPixelValue(X + 1, Y - 1);
            values[7] = GetPixelValue(X , Y - 1);
      
            double sum = 0;
            for (int x = 0; x < 8; x++)
            {
                sum += Math.Abs(values[x] - GetPixelValue(X , Y));
            }
            return sum / 8;
            // get the |element - ave|
        }
        ...
        private double GetTopLeft(int X, int Y)
        {
            ...
            // this is locations values[6] = GetPixelValue(X - 1, Y - 1);
            values[0] = GetPixelValue(X, Y - 1);
            values[1] = GetPixelValue(X -1 , Y);
            double sum = 0;
            for (int x = 0; x < 2; x++)
            {
                sum += Math.Abs(values[x] - GetPixelValue(X - 1, Y - 1));
            }
            return sum / 2;
        }
        private double GetTopRight(int X, int Y)
        {
            ...
            // this is locations values[6] = GetPixelValue(X +1, Y - 1);
            values[0] = GetPixelValue(X, Y - 1);
            values[1] = GetPixelValue(X + 1, Y+1);
            double sum = 0;
            for (int x = 0; x < 2; x++)
            {
                sum += Math.Abs(values[x] - GetPixelValue(X +1, Y - 1));
        ...
        private double GetBottomLeft(int X, int Y)
        {
            ...
            double[] values = new double[3];
            // this is locations values[6] = GetPixelValue(X - 1, Y + 1);
            values[0] = GetPixelValue(X-1, Y);
            values[1] = GetPixelValue(X , Y+1);
            double sum = 0;
            for (int x = 0; x < 2; x++)
            {
                sum += Math.Abs(values[x] - GetPixelValue(X - 1, Y + 1));
        ...
        private double GetTopMiddle(int X, int Y)
        {
            ...
            // this is locations values[6] = GetPixelValue(X, Y -1);
            values[0] = GetPixelValue(X - 1, Y-1);
            values[1] = GetPixelValue(X+1, Y - 1);
            values[2] = GetPixelValue(X - 1, Y);
            values[3] = GetPixelValue(X +1, Y);
            double sum = 0;
            for (int x = 0; x < 4; x++)
            {
                sum += Math.Abs(values[x] - GetPixelValue(X, Y - 1));
            }
            return sum / 4;
        }


        private double GetLeftMiddle(int X, int Y)
        {

            // get the  adjescent values 
            double[] values = new double[4];
            // this is locations values[6] = GetPixelValue(X-1, Y);
            values[0] = GetPixelValue(X - 1, Y - 1);
            values[1] = GetPixelValue(X , Y - 1);
            values[2] = GetPixelValue(X - 1, Y+1);
            values[3] = GetPixelValue(X , Y+1);
            ...
            for (int x = 0; x < 4; x++)
            {
                sum += Math.Abs(values[x] - GetPixelValue(X - 1, Y));
            }
            return sum / 4;
        }

        private double GetRightMiddle(int X, int Y)
        {
            ...
            values[0] = GetPixelValue(X + 1, Y - 1);
            values[1] = GetPixelValue(X, Y + 1);
            values[2] = GetPixelValue(X , Y + 1);
            values[3] = GetPixelValue(X+1, Y + 1);
            double sum = 0;
            for (int x = 0; x < 4; x++)
            {
                sum += Math.Abs(values[x] - GetPixelValue(X +1, Y));
